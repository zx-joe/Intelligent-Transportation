...
class CocoKeypoints(torch.utils.data.Dataset):
    ...
    def __init__(self, root, annFile, image_transform=None, target_transforms=None,
        ...
        #self.target_img = Image.open('test/car.jpg', 'r').convert("RGB") ## Replace
        
        # got category names and corresponding cat_ids
        
        
        self.target_img = Image.open('test/car.jpg', 'r').convert("RGB")
        ...
        #num_target_image = int(0.6 * len(self.ids))
        #num_background_image = int(0.4 * len(self.ids))
        
        #self.ids = self.ids[:num_target_image]
        #self.background_ids = self.ids[-num_background_image:]
